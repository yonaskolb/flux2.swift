...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate tiny Flux2 Klein CFG fixtures (cond/uncond -> transformer -> VAE)."
    )
    parser.add_argument(
        "--out-dir",
        default="fixtures/flux2_tiny_klein_pipeline_cfg",
        help="Output directory for the klein CFG snapshot and fixtures.",
    )
    parser.add_argument(
        "--pipeline-snapshot",
...
def main() -> None:
    ...
    inputs_out = out_dir / "klein_cfg_inputs.safetensors"
    expected_out = out_dir / "klein_cfg_expected.safetensors"
    ...
    print(f"Wrote klein cfg inputs to {inputs_out}")
    print(f"Wrote klein cfg expected outputs to {expected_out}")
