...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate tiny Flux2 multi-step denoise loop fixtures."
    )
    parser.add_argument(
        "--out-dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of inference steps for the loop.",
    )
    parser.add_argument(
        "--dtype",
        default="float32",
...
def main() -> None:
    ...
    inputs_out = out_dir / "denoise_loop_inputs.safetensors"
    expected_out = out_dir / "denoise_loop_expected.safetensors"

    save_file(
        {
            "latents": latents_history[0].contiguous().cpu(),
            "encoder_hidden_states": encoder_hidden_states.contiguous().cpu(),
            "timestep": timesteps.contiguous().cpu(),
    ...
    save_file(
        {
            "noise_preds": torch.stack(noise_preds, dim=0).contiguous().cpu(),
            "latents_history": torch.stack(latents_history, dim=0).contiguous().cpu(),
        },
        str(expected_out),
    )
