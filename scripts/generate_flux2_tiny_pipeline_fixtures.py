...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate tiny Flux2 pipeline fixtures with aligned VAE/transformer."
    )
    parser.add_argument(
        "--out-dir",
    ...
    parser.add_argument(
        ...
        help="Source snapshot for transformer/scheduler configs.",
    )
    parser.add_argument(
        "--inputs",
...
def main() -> None:
    ...
    with (vae_dir / "config.json").open("w", encoding="utf-8") as f:
        json.dump(vae_config, f, indent=2, sort_keys=True)

    vae = AutoencoderKLFlux2(**vae_config).to(device=device, dtype=dtype)
    vae.eval()
    vae_weights = {k: v.cpu() for k, v in vae.state_dict().items()}
    save_file(vae_weights, str(vae_dir / "model.safetensors"))
