...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate tiny Flux2 Klein fixtures with image conditioning."
    )
    parser.add_argument(
        "--out-dir",
        default="fixtures/flux2_tiny_klein_image_pipeline",
        help="Output directory for the klein image-conditioned snapshot and fixtures.",
    )
    parser.add_argument(
        "--pipeline-snapshot",
...
def main() -> None:
    ...
    inputs_out = out_dir / "klein_image_inputs.safetensors"
    expected_out = out_dir / "klein_image_expected.safetensors"
    ...
    print(f"Wrote klein image inputs to {inputs_out}")
    print(f"Wrote klein image expected outputs to {expected_out}")
